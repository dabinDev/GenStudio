import type { Adapter, Capability, ModelDefinition } from "@/lib/types";
...
export const TEXT_TEMPLATES: PromptTemplate[] = [
  {
    ...
    label: "品牌短文案",
    prompt:
      "请基于给定关键词，输出 5 组简洁有力的品牌短文案，每组控制在 18 个字以内，并附一句风格说明。",
  },
  {
    id: "text-storyboard",
    label: "视频脚本",
    prompt:
      "请把需求整理成分镜脚本，按镜头编号、画面内容、旁白、镜头运动、时长输出，适合直接喂给视频模型。",
  },
  {
    id: "text-image-prompt",
    label: "图片提示词",
    prompt:
      "请把需求改写成适合图像模型的高质量提示词，输出中英双语版本，并补充构图、光影、材质与镜头语言。",
  },
  {
    id: "text-video-prompt",
    label: "视频提示词",
    prompt:
      "请把需求改写成适合视频模型的生成提示词，突出主体动作、镜头运动、节奏、景别、风格和时长建议。",
  },
];
...
export const IMAGE_TEMPLATES: PromptTemplate[] = [
  {
    ...
    label: "电商海报",
    prompt:
      "生成一张高级电商海报，突出主体产品，画面干净，主次分明，保留大面积可排版留白，强调质感光影。",
  },
  {
    id: "image-cinema",
    label: "电影感剧照",
    prompt:
      "生成一张电影感剧照，具有强烈情绪氛围、明确景别、胶片级光影与真实材质细节。",
  },
  {
    id: "image-character",
    label: "角色设定",
    prompt:
      "生成人物角色设定图，完整交代服装、姿态、表情、材质与世界观氛围，风格统一且可持续扩展。",
  },
  {
    id: "image-realism",
    label: "写实增强",
    prompt:
      "参考输入素材，输出更写实、更精致的画面，保留主体核心特征，提升光线、层次与质感。",
  },
];
...
export const VIDEO_TEMPLATES: PromptTemplate[] = [
  {
    ...
    label: "产品广告",
    prompt:
      "第一人称产品广告镜头：开场特写建立质感，中段通过切换景别展示功能，结尾定格品牌记忆点。",
  },
  {
    id: "video-motion",
    label: "人物动作",
    prompt:
      "人物自然向前移动并与镜头产生互动，动作流畅，表情自然，镜头带轻微推进，氛围真实高级。",
  },
  {
    id: "video-transition",
    label: "首尾帧过渡",
    prompt:
      "让首帧与尾帧之间形成自然、连贯、节奏稳定的视觉过渡，避免跳变，突出镜头连续性。",
  },
  {
    id: "video-travel",
    label: "氛围旅拍",
    prompt:
      "生成旅拍质感短片，镜头有节奏地切换远景、中景和特写，画面通透，风格统一，动作自然。",
  },
];
